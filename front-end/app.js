...
function App() {
  ...
  const [activeTab, setActiveTab] = useState(
    persisted?.activeTab ?? DEFAULT_TAB,
  );
  const [activeFilter, setActiveFilter] = useState(
    persisted?.activeFilter ?? DEFAULT_FILTER,
  );
  const [savedIds, setSavedIds] = useState(
    new Set(persisted?.saved ?? DEFAULT_SAVED),
  );
  const [likedIds, setLikedIds] = useState(
    new Set(persisted?.liked ?? DEFAULT_LIKED),
  );
  const [threadState, setThreadState] = useState(cloneThreads);
  const [selectedSizes, setSelectedSizes] = useState(() =>
    Object.fromEntries(
      products.map((product) => [product.id, product.selectedSize]),
    ),
  );
  ...
  return (
    <div className="app-shell">
      ...
      <ProductModal
        ...
        onSelectSize={(size) =>
          activeProduct && handleSelectSize(activeProduct.id, size)
        }
        onShop={() =>
          activeProduct &&
          window.open(activeProduct.shopUrl, "_blank", "noopener,noreferrer")
        }
      />
      ...
      <StoryViewer
        ...
        onNext={() =>
          setActiveStoryId(resolvedStories[activeStoryIndex + 1]?.id ?? null)
        }
        onPrevious={() =>
          setActiveStoryId(resolvedStories[activeStoryIndex - 1]?.id ?? null)
        }
